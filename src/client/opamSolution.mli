...
(** As {!install_depexts}, but supplied with a set of system packages to be
    installed. *)
val install_sys_packages: confirm:bool -> ?nixpkgsHash:string -> sys_packages:OpamSysPkg.Set.t ->
  required:OpamSysPkg.Set.t -> OpamStateTypes.gt_variables -> OpamFile.Config.t
  -> rw switch_state option -> rw switch_state option
